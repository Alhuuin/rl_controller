...
void utils::start_rl_state(mc_control::fsm::Controller& ctl_, std::string state_name) {
  ...
  ctl.gui()->addElement(
      ...
      mc_rtc::gui::Label("Policy Loaded",
                         [&ctl]() { return ctl.rlPolicy_->isLoaded() ? "Yes" : "No"; }),
      mc_rtc::gui::Label("Observation Size",
                         [&ctl]() { return std::to_string(ctl.rlPolicy_->getObservationSize()); }),
      mc_rtc::gui::Label("Action Size",
                         [&ctl]() { return std::to_string(ctl.rlPolicy_->getActionSize()); }));
}
...
Eigen::VectorXd utils::getCurrentObservation(mc_control::fsm::Controller& ctl_) {
  auto& ctl = static_cast<RLController&>(ctl_);
  // Observation: [base angular velocity (3), roll (1), pitch (1), joint pos (10), joint vel (10),
  // past action (10), sin(phase) (1), cos(phase) (1), command (3)]
  ...
  case 0: {
    ...
    Eigen::VectorXd reorderedPos =
        ctl.policySimulatorHandling_->reorderJointsToSimulator(ctl.currentPos, ctl.dofNumber);
    Eigen::VectorXd reorderedVel =
        ctl.policySimulatorHandling_->reorderJointsToSimulator(ctl.currentVel, ctl.dofNumber);
    ...
    for (size_t i = 0; i < ctl.usedJoints_simuOrder.size(); ++i) {
      ...
      if (idx >= reorderedPos.size()) {
        mc_rtc::log::error("Leg joint index {} out of bounds for reordered size {}", idx,
                           reorderedPos.size());
        ctl.legPos(i) = 0.0;
        ctl.legVel(i) = 0.0;
      } else {
    ...
    for (size_t i = 0; i < ctl.usedJoints_simuOrder.size(); ++i) {
      ...
      if (idx >= ctl.a_simuOrder.size()) {
        mc_rtc::log::error("Past action index {} out of bounds for size {}", idx,
                           ctl.a_simuOrder.size());
        ctl.legAction(i) = 0.0;
      } else {
  ...
  case 3: {
    ...
    Eigen::VectorXd reorderedVel =
        ctl.policySimulatorHandling_->reorderJointsToSimulator(ctl.currentVel, ctl.dofNumber);
    Eigen::VectorXd reorderedQZero =
        ctl.policySimulatorHandling_->reorderJointsToSimulator(ctl.q_zero_vector, ctl.dofNumber);
    for (size_t i = 0; i < ctl.usedJoints_simuOrder.size(); ++i) {
      int idx = ctl.usedJoints_simuOrder[i];
      if (idx >= reorderedPos.size()) {
        mc_rtc::log::error("Leg joint index {} out of bounds for reordered size {}", idx,
    ...
    for (size_t i = 0; i < ctl.usedJoints_simuOrder.size(); ++i) {
      ...
      if (idx >= ctl.a_simuOrder.size()) {
        ...
        ctl.legAction(i) =
            (std::abs(ctl.actionScale) > 1e-9) ? ctl.a_simuOrder(idx) / ctl.actionScale : 0.0;
      }
    }
...
bool utils::applyAction(mc_control::fsm::Controller& ctl_, const Eigen::VectorXd& action) {
  ...
  if (static_cast<size_t>(action.size()) == ctl.dofNumber)
    ...
    for (size_t i = 0; i < ctl.usedJoints_simuOrder.size(); ++i) {
      ...
      if (idx >= fullAction.size()) {
        mc_rtc::log::error("Joint index {} out of bounds for fullAction size {}", idx,
                           fullAction.size());
  ...
  if (shouldRunInference_) {
    ...
    ctl.a_vector =
        ctl.policySimulatorHandling_->reorderJointsFromSimulator(fullAction, ctl.dofNumber) *
        ctl.actionScale;
