...
struct RLController_DLLAPI RLController : public mc_control::fsm::Controller {
  ...
  void switchPolicy(int policyIndex,
                    const mc_rtc::Configuration& config); // Switch to a different policy at runtime
  ...
  createContactAnchor(const mc_rbdyn::Robot& anchorRobot, const mc_rtc::Configuration& config);

  void computeRLStateSimulated(); // Compute the state of the robot as if it was simulated with the
                                  // RL policy
  ...
  std::vector<std::string> anchorLinks =
      {}; // Ground contact points to use for the residual, in the order of the robot's joints
  ...
  Eigen::VectorXd
      current_kp; // Gains get from the robot/simulator, used to check if we need to update
  Eigen::VectorXd
      current_kd;        // Gains get from the robot/simulator, used to check if we need to update
  ...
  Eigen::VectorXd legPos_prev, legVel_prev,
      legAction_prev; // Leg position, velocity and action in mc_rtc order
  ...
  Eigen::VectorXd legPos_prev_prev, legVel_prev_prev,
      legAction_prev_prev; // Leg position, velocity and action in mc_rtc order
  ...
  std::vector<Eigen::Vector3d>
      feetForces; // Ground reaction forces for the feet, in the order of FeetForceSensors
  std::vector<std::string> residualAnchorWeightJoints =
      {}; // Joints used to weight anchor points from residual torques
  std::vector<int> residualAnchorWeightJointIndices =
      {}; // Indices in external torque vector matching residualAnchorWeightJoints
