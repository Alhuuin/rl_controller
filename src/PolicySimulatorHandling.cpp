...
PolicySimulatorHandling::PolicySimulatorHandling(const std::string& simulator_name,
  ...
  if (!mcRtcToSimuIdx_.empty()) {
    mc_rtc::log::warning("Simulator mapping for {} on {} manually initialized in header",
                         simulator_name, robot_name);
  ...
  if (robot_name == "H1" && (simulator_name == "Maniskill" || simulator_name == "IsaacLab")) {
    ...
    simuToMcRtcIdx_ = invertMapping(mcRtcToSimuIdx_);
             ...
             (simulator_name == "Maniskill" || simulator_name == "IsaacLab")) {
    ...
    mc_rtc::log::error_and_throw(
        "Unsupported simulator or robot: {} with {}, please specify a mc_rtc to simulator joint "
        "order mapping in PolicySimulatorHandling.h",
        simulator_name, robot_name);
  }
}
...
Eigen::VectorXd PolicySimulatorHandling::reorderJointsToSimulator(const Eigen::VectorXd& obs,
                                                                  size_t dofNumber) {
  ...
  for (size_t i = 0; i < dofNumber; i++) {
    if (i >= mcRtcToSimuIdx_.size()) {
      mc_rtc::log::error("Trying to access mcRtcToSimuIdx_[{}] but size is {}", i,
                         mcRtcToSimuIdx_.size());
      reordered[i] = 0.0;
      continue;
    }
...
Eigen::VectorXd PolicySimulatorHandling::reorderJointsFromSimulator(const Eigen::VectorXd& action,
                                                                    size_t dofNumber) {
  ...
  for (size_t i = 0; i < dofNumber; i++) {
    if (i >= simuToMcRtcIdx_.size()) {
      mc_rtc::log::error("Trying to access simuToMcRtcIdx_[{}] but size is {}", i,
                         simuToMcRtcIdx_.size());
      reordered[i] = 0.0;
      continue;
    }
    ...
    if (srcIdx >= action.size()) {
      mc_rtc::log::error("Action reorder index {} out of bounds for action size {}", srcIdx,
                         action.size());
      reordered[i] = 0.0;
    } else {
...
std::vector<int> PolicySimulatorHandling::getSimulatorIndices(std::vector<int> mcRtcIndices) const {
  ...
  for (int idx : mcRtcIndices) {
    if (static_cast<size_t>(idx) < simuToMcRtcIdx_.size()) {
      ...
      mc_rtc::log::error("Index {} out of bounds for simuToMcRtcIdx_ size {}", idx,
                         simuToMcRtcIdx_.size());
    }
  }
  return simuIndices;
