...
RLPolicyInterface::RLPolicyInterface(const std::string& policyPath)
    : inputSize_(0), outputSize_(0), inputIs2D_(false), outputIs2D_(false), isLoaded_(false),
...
void RLPolicyInterface::loadPolicy(const std::string& path) {
  ...
  try {
    ...
    outputNamePtr_ = outputName_.c_str();
    ...
    case 1:
      ...
      inputIs2D_ = false;
      mc_rtc::log::info("Raw input shape from model: [{}] (1D - implicit batch size)",
                        inputShape[0]);
      break;
    ...
    case 2:
      ...
      inputIs2D_ = true;
      mc_rtc::log::info("Raw input shape from model: [{}, {}] (2D - explicit batch size)",
      ...
      if (inputShape[0] == 1 || inputShape[0] == -1) {
        ...
        inputSize_ = static_cast<int>(inputShape[1]);
        mc_rtc::log::warning(
            "Ambiguous input shape [{}, {}], assuming standard format [batch_size, obs_size]",
            inputShape[0], inputShape[1]);
      }
      break;
    ...
    default:
      mc_rtc::log::error(
          "Input shape should be 1D [obs_size] or 2D [batch_size, obs_size], got {}D",
          inputShape.size());
      isLoaded_ = false;
      return;
    }
    ...
    case 1:
      ...
      outputIs2D_ = false;
      mc_rtc::log::info("Raw output shape from model: [{}] (1D - implicit batch size)",
                        outputShape[0]);
      break;
    ...
    case 2:
      ...
      outputIs2D_ = true;
      mc_rtc::log::info("Raw output shape from model: [{}, {}] (2D - explicit batch size)",
      ...
      if (outputShape[0] == 1 || outputShape[0] == -1) {
        ...
        outputSize_ = static_cast<int>(outputShape[1]);
        mc_rtc::log::warning(
            "Ambiguous output shape [{}, {}], assuming standard format [batch_size, action_size]",
            outputShape[0], outputShape[1]);
      }
      break;
    ...
    default:
      mc_rtc::log::error(
          "Output shape should be 1D [action_size] or 2D [batch_size, action_size], got {}D",
          outputShape.size());
      isLoaded_ = false;
      return;
    }
    ...
    Ort::Value testInputTensor =
        Ort::Value::CreateTensor<float>(*memoryInfo_, testInput.data(), testInput.size(),
                                        testInputShape.data(), testInputShape.size());

    auto testOutputTensors = onnxSession_->Run(Ort::RunOptions{nullptr}, &inputNamePtr_,
                                               &testInputTensor, 1, &outputNamePtr_, 1);
...
Eigen::VectorXd RLPolicyInterface::predict(const Eigen::VectorXd& observation) {
  ...
  if (observation.size() != getObservationSize()) {
    mc_rtc::log::error("Observation size mismatch: expected {}, got {}", getObservationSize(),
                       observation.size());
    return Eigen::VectorXd::Zero(outputSize_);
  }
...
Eigen::VectorXd RLPolicyInterface::runOnnxInference(const Eigen::VectorXd& observation) {
  ...
  // Run inference
  auto outputTensors = onnxSession_->Run(Ort::RunOptions{nullptr}, &inputNamePtr_, &inputTensor, 1,
                                         &outputNamePtr_, 1);

  if (outputTensors.size() != 1) {
    throw std::runtime_error("Expected 1 output tensor, got " +
  ...
  if (outputShape.size() == 1 and outputShape[0] != getActionSize()) {
    throw std::runtime_error("Output action size mismatch: expected " +
                             std::to_string(getActionSize()) + ", got " +
                             std::to_string(outputShape[0]));
    ...
    if (outputActionSize != getActionSize()) {
      throw std::runtime_error("Output action size mismatch: expected " +
                               std::to_string(getActionSize()) + ", got " +
                               std::to_string(outputActionSize));
    }
  } else {
    throw std::runtime_error("Expected 1D or 2D output tensor, got " +
                             std::to_string(outputShape.size()) + "D");
  }
