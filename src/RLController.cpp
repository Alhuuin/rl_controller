...
RLController::RLController(mc_rbdyn::RobotModulePtr rm, double dt,
  ...
  dynamicsConstraint =
      mc_rtc::unique_ptr<mc_solver::DynamicsConstraint>(new mc_solver::DynamicsConstraint(
...
bool RLController::run() {
  ...
  if (useResidual) {
    if (robotName == "H1") {
      leftAnklePos =
          robot().mbc().bodyPosW[robot().bodyIndexByName("left_ankle_link")].translation();
      rightAnklePos =
          robot().mbc().bodyPosW[robot().bodyIndexByName("right_ankle_link")].translation();
  ...
  if (!useQP) // Run RL without taking the QP into account
  {
    ...
    tau_cmd = (pd_gains_ratio * kp_vector).cwiseProduct(q_rl - currentPos) -
              (pd_gains_ratio * kd_vector).cwiseProduct(currentVel);
...
void RLController::RLuseJoyStickInputs() {
  ...
  auto& stickFunc =
      datastore().get<std::function<Eigen::Vector2d(joystickAnalogicInputs)>>("Joystick::Stick");
  ...
  // Read D-pad buttons
  DirectionButtons = {
      datastore().get<bool>("Joystick::UpPad"), datastore().get<bool>("Joystick::DownPad"),
...
void RLController::RLuseKeyboardInputs() {
  ...
  if (n > 0 && k.sz + n < 64)
    std::copy(tmp, tmp + n, k.buf.begin() + k.sz), k.sz += n;
...
void RLController::loadConfig(const mc_rtc::Configuration& config) {
  ...
  if (useForceSensors) {
    ...
    feetForceSensors.empty()
        ? mc_rtc::log::error_and_throw("please specify force_sensors in yaml for your Robot.")
        : mc_rtc::log::info("Using feet force sensors");
  ...
  if (useResidual) {
    const auto& confAnchorLinks =
        config("Robot")(robotName)("anchor_links", std::vector<std::string>{});
    if (!confAnchorLinks.empty()) {
      anchorLinks = confAnchorLinks;
    }

    residualAnchorWeightJoints =
        config("Robot")(robotName)("knee_joints", std::vector<std::string>{});
    ...
    for (const auto& jointName : residualAnchorWeightJoints) {
      ...
      if (jointIndex < 0) {
        mc_rtc::log::warning(
            "[RLController] residual_anchor_weight_joints contains unknown joint {}", jointName);
        continue;
      }
      residualAnchorWeightJointIndices.push_back(jointIndex + 5);
    }

    if (!residualAnchorWeightJointIndices.empty() &&
        residualAnchorWeightJointIndices.size() < anchorLinks.size()) {
      mc_rtc::log::warning(
          "[RLController] residual_anchor_weight_joints has {} entries, but anchorLinks has {}. "
          "Falling back to geometric averaging for missing weights.",
          residualAnchorWeightJointIndices.size(), anchorLinks.size());
    }
  }
...
void RLController::tasksComputation(Eigen::VectorXd& currentTargetPosition) {
  ...
  tau_d = (pd_gains_ratio * kp_vector).cwiseProduct(currentTargetPosition - currentPos) -
          (pd_gains_ratio * kd_vector).cwiseProduct(currentVel);
...
void RLController::computeInversePD() {
  ...
  Eigen::VectorXd tau_cmd_w_floatingBase =
      M_w_floatingBase * ddot_qp_w_floatingBase + Cg_w_floatingBase - extTorques;
  ...
  qddot_rl_simulatedMeasure = M_w_floatingBase.llt()
                                  .solve(tau_rl_w_floating_base + extTorques - Cg_w_floatingBase)
                                  .tail(dofNumber);
  ...
  q_cmd =
      currentPos +
      Kp_inv * (tau_cmd + (pd_gains_ratio * kd_vector)
                              .cwiseProduct(currentVel)); // Inverse PD control to get the commanded
                                                          // position <=> RL position control
}
...
void RLController::computeRLStateSimulated() {
  ...
  tau_rl = (pd_gains_ratio * kp_vector).cwiseProduct(q_rl - currentPos) -
           (pd_gains_ratio * kd_vector).cwiseProduct(currentVel);
  Eigen::VectorXd tau_rl_w_floating_base = Eigen::VectorXd::Zero(robot().mb().nrDof());
  tau_rl_w_floating_base.tail(dofNumber) = tau_rl;
  Eigen::VectorXd content = tau_rl_w_floating_base + extTorques -
                            Cg_w_floatingBase; // Add the external torques to the desired torques
...
void RLController::addLog() {
  ...
  logger().addLogEntry("RLController_qddot_qp_w_floatingBase",
                       [this]() { return ddot_qp_w_floatingBase; });
  logger().addLogEntry("RLController_tau_cmd", [this]() { return tau_cmd; });

  // RL variables
  logger().addLogEntry("RLController_RL_q", [this]() { return q_rl; });
  logger().addLogEntry("RLController_RL_qSimulatedMeasure",
                       [this]() { return q_rl_simulatedMeasure; });
  logger().addLogEntry("RLController_RL_qdotSimulatedMeasure",
                       [this]() { return qdot_rl_simulatedMeasure; });
  logger().addLogEntry("RLController_RL_qddotSimulatedMeasure",
                       [this]() { return qddot_rl_simulatedMeasure; });
  ...
  logger().addLogEntry("RLController_qdot_limHard_upper",
                       [this]() { return jointLimitsHardVel_upper; });
  logger().addLogEntry("RLController_qdot_limHard_lower",
                       [this]() { return jointLimitsHardVel_lower; });
  ...
  logger().addLogEntry("RLController_qOutNoModification",
                       [this, qOut_vec]() mutable -> const std::vector<double>& {
                         auto& robot = this->robot();
                         for (size_t i = 0; i < qOut_vec.size(); ++i) {
                           auto mbcIndex = robot.jointIndexInMBC(i);
                           if (mbcIndex != -1) {
                             qOut_vec[i] = qOut[static_cast<size_t>(mbcIndex)][0];
                           }
                         }
                         return qOut_vec;
                       });
  std::vector<double> alphaOut_vec(robot().refJointOrder().size(), 0);
  logger().addLogEntry("RLController_alphaOutNoModification",
                       [this, alphaOut_vec]() mutable -> const std::vector<double>& {
                         auto& robot = this->robot();
                         for (size_t i = 0; i < alphaOut_vec.size(); ++i) {
                           auto mbcIndex = robot.jointIndexInMBC(i);
                           if (mbcIndex != -1) {
                             alphaOut_vec[i] = alphaOut[static_cast<size_t>(mbcIndex)][0];
                           }
                         }
                         return alphaOut_vec;
                       });
  std::vector<double> tauOut_vec(robot().refJointOrder().size(), 0);
  logger().addLogEntry("RLController_tauOutNoModification",
                       [this, tauOut_vec]() mutable -> const std::vector<double>& {
                         auto& robot = this->robot();
                         for (size_t i = 0; i < tauOut_vec.size(); ++i) {
                           auto mbcIndex = robot.jointIndexInMBC(i);
                           if (mbcIndex != -1) {
                             tauOut_vec[i] = tauOut[static_cast<size_t>(mbcIndex)][0];
                           }
                         }
                         return tauOut_vec;
                       });

  logger().addLogEntry("RLController_floatingBase_qOutNoModification",
                       [this]() { return floatingBase_qOut; });
  logger().addLogEntry("RLController_floatingBase_alphaOutNoModification",
                       [this]() { return floatingBase_alphaOut; });
  logger().addLogEntry("RLController_floatingBase_tauOutNoModification",
                       [this]() { return floatingBase_tauOut; });
  logger().addLogEntry("RLController_floatingBase_qOutPD",
                       [this]() { return floatingBase_qOutPD; });
  logger().addLogEntry("RLController_floatingBase_alphaOutPD",
                       [this]() { return floatingBase_alphaOutPD; });
  logger().addLogEntry("RLController_floatingBase_tauOutPD",
                       [this]() { return floatingBase_tauOutPD; });
  logger().addLogEntry("RLController_floatingBase_qIn", [this]() { return floatingBase_qIn; });
  logger().addLogEntry("RLController_floatingBase_alphaIn",
                       [this]() { return floatingBase_alphaIn; });
...
void RLController::addGui(const mc_rtc::Configuration& config) {
  ...
  gui()->addElement(
      ...
      mc_rtc::gui::Label(
          "Current policy",
          [this]() -> const std::string& { return policyPaths[currentPolicyIndex]; }),
      mc_rtc::gui::ComboInput(
          "Select policy", policyPaths,
          [this]() -> const std::string& { return policyPaths[currentPolicyIndex]; },
          [this,
           config](const std::string& selected) { // Capture config by VALUE (makes a safe copy)
  ...
  gui()->addElement(
      {"RLController", "Policy"}, mc_rtc::gui::Button("Reload current policy", [this, config]() {
...
void RLController::initializeRobot(const mc_rtc::Configuration& config) {
  ...
  jointLimitsPos_upper =
      jointLimitsHardPos_upper - (jointLimitsHardPos_upper - jointLimitsHardPos_lower) * dsPercent;
  jointLimitsPos_lower =
      jointLimitsHardPos_lower + (jointLimitsHardPos_upper - jointLimitsHardPos_lower) * dsPercent;
  ...
  ddot_qp_w_floatingBase = Eigen::VectorXd::Zero(
      robot().mb().nrDof()); // Desired acceleration in the QP solver with floating base

  tau_cmd = Eigen::VectorXd::Zero(dofNumber); // Final torque that control the robot
  q_cmd = Eigen::VectorXd::Zero(
      dofNumber); // The commended position send to the internal PD of the robot
  ...
  for (const auto& j : robot().mb().joints()) {
    ...
    if (j.type() == rbd::Joint::Type::Rev) {
      ...
      if (const auto& t = posture[robot().jointIndexByName(joint_name)]; !t.empty()) {
        ...
        mc_rtc::log::info("[RLController] Joint {}: currentTargetPosition {}, kp {}, kd {}",
                          joint_name, q_rl[i], kp_vector[i], kd_vector[i]);
        i++;
      }
    }
  }
  ...
  if (!datastore().has("anchorFrameFunction") && (useResidual || useForceSensors)) {
    datastore().make_call("anchorFrameFunction",
                          [this, &config](const mc_rbdyn::Robot& real_robot) {
                            return createContactAnchor(real_robot, config);
                          });
  }
...
void RLController::configRL(const mc_rtc::Configuration& config) {
  mc_rtc::log::info("Loading RL policy [{}]: {}", currentPolicyIndex,
                    policyPaths[currentPolicyIndex]);
  try {
    ...
    if (rlPolicy_) {
      ...
      mc_rtc::log::info("Initialized observation vector with size: {}",
                        rlPolicy_->getObservationSize());
    } else {
  ...
  usedJoints_mcRtcOrder =
      config("policies")[currentPolicyIndex]("used_joints_index", std::vector<int>{});
...
std::tuple<Eigen::VectorXd, Eigen::VectorXd> RLController::getPDGains() {
  ...
  Eigen::VectorXd p_vec =
      Eigen::VectorXd::Map(proportionalGains_vec.data(), proportionalGains_vec.size());
...
bool RLController::setPDGains(Eigen::VectorXd p_vec, Eigen::VectorXd d_vec) {
  ...
  const std::vector<double> proportionalGains_vec(actual_kp.data(),
                                                  actual_kp.data() + actual_kp.size());
  const std::vector<double> dampingGains_vec(actual_kd.data(), actual_kd.data() + actual_kd.size());

  bool res =
      datastore().call<bool>(robot_name + "::SetPDGains", proportionalGains_vec, dampingGains_vec);
  if (res) {
...
bool RLController::gainsUpdateRequired(double tol) {
  ...
  bool gainCorrectionRequired =
      ((current_kp - target_kp).norm() >= tol) || ((current_kd - target_kd).norm() >= tol);
  if (gainCorrectionRequired) {
    ...
    mc_rtc::log::info("[RLController] target_kp (ratio {}): {}", pd_gains_ratio,
                      target_kp.transpose());
    mc_rtc::log::info("[RLController] target_kd (ratio {}): {}", pd_gains_ratio,
                      target_kd.transpose());
  }
...
std::pair<sva::PTransformd, Eigen::Vector3d>
RLController::createContactAnchor(const mc_rbdyn::Robot& anchorRobot,
                                  const mc_rtc::Configuration& config) {
  ...
  if (totalFootForceZ > minFootForceForAnchor) {
    contact_anchor_tf =
        sva::PTransformd(Eigen::Matrix3d::Identity(), weightedAnchor / totalFootForceZ);
    oneFootForceRatio = 1.0 / static_cast<double>(feetForceSensors.size());
  ...
  if (!hasForceBasedRatio && useResidual) {
    ...
    const size_t weightedContacts =
        std::min(anchorLinks.size(), residualAnchorWeightJointIndices.size());
  ...
  if (anchorPositions.empty()) {
    mc_rtc::log::warning(
        "[RLController] No anchorLinks available, please specify them in the controller config");
  ...
  if (anchorPositions.size() == 2) {
    ...
    const Eigen::Vector3d contact_anchor =
        anchorPositions[0] * (1 - oneFootForceRatio) + anchorPositions[1] * oneFootForceRatio;
    const Eigen::Vector3d anchor_vel =
        anchorVels[0] * (1 - oneFootForceRatio) + anchorVels[1] * oneFootForceRatio;
    contact_anchor_tf = sva::PTransformd(Eigen::Matrix3d::Identity(), contact_anchor);
    return {contact_anchor_tf, anchor_vel};
  }
